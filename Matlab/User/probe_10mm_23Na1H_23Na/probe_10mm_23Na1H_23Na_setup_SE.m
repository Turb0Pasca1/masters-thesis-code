% probe_10mm_23Na1H_23Na_setup_SE.m

%% initialize system
% run script in main openMATLAB folder containing all files
LoadSystem probe_10mm_23Na1H_23Na;
...
% sufficient
iterations                          = 1; 
% for 10mm saturated NaCl CuSO4 sample 
T1_estimate                         = 40e-3;    
...
%% imaging parameters
% for probe_10mm_23Na1H_23Na saturated NaCl CuSO4 sample
...
% measured using Demo_T2.m for saturated NaCl CuSO4 sample
...
% combination of tEcho = 8e-3 and HzPerPixMin = 400 exceeds gradient power
% limitation
% -> reduce HzPerPixMin = 250 to avoid gradient limit
% -> increase tEcho = 10e-3 to allow for proper timing;
Seq.tEcho                           = 10e-3;
...
savenameImage   = sprintf('User/%s/double_res_%s_%s_avg_%d_%s_Image.csv', HW.UserName, mode, sample, Seq.average, pic)
savenamekSpace  = sprintf('User/%s/double_res_%s_%s_avg_%d_%s_kSpace.csv', HW.UserName, mode, sample, Seq.average, pic)
